...
def generate_pocket(data_dir, cid, ligand_filename, protein_filename, distance=5):
    ...
    if not os.path.exists(pocket_path_full):
        ...
        pymol.cmd.select(
            "Pocket",
            f"byres {delete_extension_from_filename(ligand_filename)} around {distance}",
        )
...
def generate_complex(data_dir, cid, ligand_filename, pocket_path_full, distance=5):
    ...
    if input_ligand_format != "pdb":
        ...
        ligand_path = os.path.join(
            complex_dir, delete_extension_from_filename(ligand_filename) + ".pdb"
        )
        # os.system(f'obabel {ligand_input_path} -O {ligand_path} -d')
        obConversion = openbabel.OBConversion()
        obConversion.SetInAndOutFormats(input_ligand_format, "pdb")
    ...
    if ligand == None:
        raise RuntimeError(
            f"Failed to generate complex: Unable to process ligand of {cid}"
        )

    pocket = Chem.MolFromPDBFile(pocket_path_full, removeHs=True)
    if pocket == None:
        raise RuntimeError(
            f"Failed to generate complex: Unable to process pocket of {cid}"
        )
...
def generate_pocket_and_complex(
    data_dir, cid, ligand_filename, protein_filename, distance=5
):
    """
    ...
    pocket_path_full = generate_pocket(
        data_dir, cid, ligand_filename, protein_filename, distance=distance
    )
    complex_path_full = generate_complex(
        data_dir, cid, ligand_filename, pocket_path_full, distance=distance
    )
